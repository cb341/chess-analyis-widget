(function () {
  ...
  class ChessWidget extends HTMLElement {
    ...
    goTo(ply) {
      if (!this.game) return;
      this.currentPly = Math.max(
        0,
        Math.min(ply, this.game.positions.length - 1),
      );
      this.render();
    }
    ...
    render() {
      ...
      this.innerHTML = [
        ...
        '<div class="cw-stage">',
        this.renderEval(position),
        this.renderBoard(position),
        "</div>",
        '<aside class="cw-side">',
        this.renderHeader(),
        this.renderControls(),
    ...
    renderHeader() {
      ...
      return (
        "<h2>" +
        escapeHtml(metadata.White || "White") +
        " vs " +
        escapeHtml(metadata.Black || "Black") +
        "</h2>" +
        "<p>" +
        escapeHtml(metadata.Result || "*") +
        "</p>" +
        "<p>" +
        escapeHtml(this.game.summary || "") +
        "</p>"
      );
    }
    ...
    renderControls() {
      return (
        ...
        (this.currentPly === 0 ? " disabled" : "") +
        ">Previous</button>" +
        '<button type="button" data-action="next"' +
        (this.currentPly >= this.game.positions.length - 1 ? " disabled" : "") +
        ">Next</button>" +
        "</div>"
      );
    }
    ...
    renderCurrent(position) {
      var annotation = position.annotation
        ? LABELS[position.annotation] || position.annotation
        : "";
      return (
        '<div class="cw-current" aria-live="polite">' +
        (position.san ? escapeHtml(position.san) : "Starting position") +
        "</div>" +
        '<div class="cw-annotation cw-annotation-' +
        escapeHtml(position.annotation || "none") +
        '">' +
    ...
    renderEval(position) {
      ...
      return (
        '<div class="cw-eval" aria-label="Evaluation">' +
        '<div class="cw-eval-black" style="height:' +
        (100 - white) +
        '%"></div>' +
        '<div class="cw-eval-white" style="height:' +
        white +
        '%"></div>' +
        "</div>"
      );
    ...
    renderBoard(position) {
      ...
      for (var rank = 8; rank >= 1; rank--) {
        for (var file = 0; file < FILES.length; file++) {
          ...
          var last =
            position.last_move &&
            (position.last_move.from === square ||
              position.last_move.to === square);
          html.push(
            '<div class="cw-square ' +
              (isLight ? "cw-light" : "cw-dark") +
              (last ? " cw-last" : "") +
              '" aria-label="' +
              square +
              '">' +
    ...
    moveButton(move) {
      return (
        '<button type="button" data-ply="' +
        move.ply +
        '" aria-current="' +
        (move.ply === this.currentPly ? "true" : "false") +
        '">' +
        escapeHtml(move.san + (MARKS[move.annotation] || "")) +
        "</button>"
      );
    }
    ...
    bindButtons() {
      ...
      this.querySelectorAll("[data-action]").forEach(function (button) {
        button.addEventListener("click", function () {
          self[button.dataset.action]();
        });
      });
      this.querySelectorAll("[data-ply]").forEach(function (button) {
        button.addEventListener("click", function () {
          self.goTo(Number(button.dataset.ply));
        });
      });
    }
